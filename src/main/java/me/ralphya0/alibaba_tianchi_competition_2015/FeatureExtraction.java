...
public class FeatureExtraction {
    ...
    public static void main(String[] args) {
        ...
        final List<Integer> split_dates_or = new ArrayList<Integer>();
        final Map<Integer,String> split_d = new HashMap<Integer,String>();
        ...
        try{
            ...
            if(args != null && args.length > 0){
                //master = args[0].trim();
                input = args[0].trim();
                item_input = args[1].trim();
                split_date = args[2].trim();
                System.out.println("中文--spark-submit 参数解析完毕 ： " + input + "," + item_input + "," + split_date);
            }
            ...
            if(split_date != null && !split_date.equals("null")){
                ...
                if(ls != null ){
                    ...
                    while(day < 20){
                        int hour = 0;
                        hour = 13 * 24 + (day - 1) * 24;
                        split_d.put(hour,ls[0] + day);
                        split_dates_or.add(hour);
                        day ++;
                    }
                    System.out.println("中文--split_dates生成完毕");
                    for(int i : split_dates_or){
                        System.out.println(split_d.get(i) + "-" + i);
                    }
                }
            }
            
            SparkConf conf = new SparkConf().setAppName("feature-extraction-yx").setMaster("spark://tianchi-node1:7077");
            JavaSparkContext sc = new JavaSparkContext(conf);
            
            //其他需要使用的变量
            final Broadcast<List<Integer>> split_dates_ordered = sc.broadcast(split_dates_or);
            final Broadcast<Map<Integer,String>> split_dates = sc.broadcast(split_d);
...
//                                    
//                                    for(int s : split_dates_ordered.value()){
...
//                          for(int split_hour : split_dates_ordered.value()){
...
//                                        for(Integer split_h : split_dates_ordered.value()){
...
//                                                int split_day_int = Integer.parseInt(split_dates.value().get(split_h));
//                                                
//                                                
...
//                }
//                BufferedWriter bw = new BufferedWriter(new FileWriter(output_prefix + "split_day_" + split_dates.value().get(arg0._1) + ".csv"));
...
//            }});
    ...
    //对原始输入文件的过滤功能被分解到该函数中
    public void input_filter(){
      //垂直商品
        ...
        try{
            ...
            String output = "/home/tianchi/project-base/tianchi/yaoxin/result/train_user_filtered.csv";
                //初始化垂直商品列表
                ...
                System.out.println("第一条记录为 " + lines.first());
                ...
                bw.write(sb.toString());
                bw.close();
                System.out.println("过滤结果已写入输出文件");
        }
        catch(Exception e){
            e.printStackTrace();
        }
    ...
    //一次运行只提取一个split_date的各项特征
    public void feature_extraction(String split_date){
